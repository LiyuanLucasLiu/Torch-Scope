...
class wrapper():
    ...
    def add_model_parameter_stats(self, model, batch_index, save=False):
        ...
        for name, param in model.named_parameters():
            ...
            self.writer.add_scalar("parameter_mean/" + name, param.data.mean(),batch_index)
            self.writer.add_scalar("parameter_std/" + name, param.data.std(), batch_index)
    ...
    def add_model_update_stats(self, model, batch_index)
	    ...
	    for name, param in model.named_parameters():
	        if param.requires_grad:
	            ...
	            self.writer.add_scalar("gradient_update/" + name, update_norm / (param_norm + 1e-7), batch_index)

    def add_model_parameter_histograms(self, model, batch_index) -> None:
        for name, param in model.named_parameters():
        	if param.requires_grad:
